    ...
    with app.app_context():
        try:
            ...
            result = db.session.execute(
                text("UPDATE gallery_photo SET image = REPLACE(image, '/static/uploads/', '/uploads/')")
            )
            print(f"Обновлено gallery_photo: {result.rowcount} записей")
            
            # Обновить пути в документах
            result = db.session.execute(
                text("UPDATE document SET file_path = REPLACE(file_path, '/static/uploads/', '/uploads/')")
            )
            print(f"Обновлено document: {result.rowcount} записей")
            
            # Обновить пути в фото номеров
            result = db.session.execute(
                text("UPDATE room_image SET image = REPLACE(image, '/static/uploads/', '/uploads/')")
            )
